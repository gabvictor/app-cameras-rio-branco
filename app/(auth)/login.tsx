...
import { auth } from '../../core/firebaseConfig'; // Caminho atualizado

// --- (O resto do seu ficheiro de login continua igual a partir daqui) ---
// --- COMPONENTE VISUAL PARA O CAMPO DE INPUT (COM ÍCONE) ---
interface AuthInputProps {
  ...
  const AuthInput = ({ iconName, placeholder, value, onChangeText, secureTextEntry = false, isDarkTheme, keyboardType = 'default' }: AuthInputProps) => {
  ...
  // --- COMPONENTE PRINCIPAL DO ECRÃ DE LOGIN ---
  export default function LoginScreen() {
      ...
      signInWithEmailAndPassword(auth, email, password)
        .then(userCredential => {
          ...
          if (!user.emailVerified) {
            // Desloga imediatamente se e-mail não verificado
            auth.signOut();
            setError('Por favor, verifique seu e-mail antes de entrar.');
            setForm('verification'); // muda para tela de verificação
            return;
          }
    ...
    const handleRegister = () => {
      ...
      if (!termsAccepted) {
        setError('Você precisa aceitar os Termos de Responsabilidade para continuar.');
        return;
      }
  ...
  if (currentUser) {
    currentUser.reload().then(() => {
      if (currentUser.emailVerified) {
        ...
        setError('E-mail ainda não verificado. Por favor, verifique sua caixa de entrada.');
      }
    });
  }
};
    ...
    const renderForm = () => {
      const titles = {
        ...
        verification: { title: 'Verifique seu E-mail', subtitle: 'Enviamos um link para o seu e-mail. Por favor, clique nele para continuar.' },
      };
      
      const { title, subtitle } = titles[form];
      const isRegisterButtonDisabled = loading || (form === 'register' && !termsAccepted);
      ...
      return (
        <View style={styles.formContainer}>
          ...
          {form !== 'verification' && (
              <TouchableOpacity 
                style={[styles.button, (form === 'register' ? isRegisterButtonDisabled : loading) && styles.buttonDisabled]}
                onPress={form === 'login' ? handleLogin : form === 'register' ? handleRegister : handlePasswordReset}
                disabled={form === 'register' ? isRegisterButtonDisabled : loading}
              >
                {loading ? (
                  <ActivityIndicator color="#fff" />
                ) : (
                  <Text style={styles.buttonText}>
                    {form === 'login' ? 'Entrar' : form === 'register' ? 'Criar Conta' : 'Enviar Link'}
                  </Text>
                )}
              </TouchableOpacity>
          )}
          ...
          <View style={styles.footer}>
             ...
             {form === 'reset' && (
              ...
              {form === 'verification' && (
                <TouchableOpacity onPress={() => switchForm('login')} style={styles.footerLinkContainer}>
                    <Text style={styles.footerText}>Verificou seu e-mail? </Text>
                    <Text style={styles.linkText}>Tentar login novamente</Text>
                </TouchableOpacity>
             )}
          </View>
        </View>
  ...
  const getDynamicStyles = (isDarkTheme: boolean) => StyleSheet.create({
    safeArea: { flex: 1, backgroundColor: isDarkTheme ? '#1e2b3b' : '#f1f5f9' },
    container: {
      flexGrow: 1,
      alignItems: 'center',
    ...
    headerIcon: {
      color: isDarkTheme ? '#94a3b8' : '#4f46e5',
    },
    title: {
      fontSize: 30,
      fontWeight: 'bold',
